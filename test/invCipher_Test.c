#include "../src/invCipher.c"
#include <stdio.h>

int test_invSubBytes();
int test_invShiftRows();
int test_invMixColumns();
...
int main(int argc, char *argv[]) {
  printf("\nTesting invCipher.c\n");
  int testsPassed = 0;
  testsPassed += test_invSubBytes();
  testsPassed += test_invShiftRows();
  testsPassed += test_invMixColumns();
  testsPassed += test_addRoundKey();
  testsPassed += test_invCipher();
  printf("Total tests passed: %d\n\n", testsPassed);
}

int test_invSubBytes() {
  printf("-- Testing test_invSubBytes ... ");
  setCurrentState(&afterSubBytesState[0][0]);
  invSubBytes();
  for ( uint8_t col = 0; col < STATE_COLS; col++ ) {
    for ( uint8_t row = 0; row < STATE_ROWS; row++ ) {
      if (state[col][row] != initialState[col][row]){
        printf("failure\n");
        return 0;
      }
    }
...
int test_invShiftRows() {
  printf("-- Testing test_invShiftRows ... ");
  setCurrentState(&afterShiftRowsState[0][0]);
  invShiftRows();
  for ( uint8_t col = 0; col < STATE_COLS; col++ ) {
    for ( uint8_t row = 0; row < STATE_ROWS; row++ ) {
...
int test_invMixColumns() {
  printf("-- Testing test_invMixColumns ... ");
  setCurrentState(&afterMixColumnsState[0][0]);
  invMixColumns();
  for ( uint8_t col = 0; col < STATE_COLS; col++ ) {
    for ( uint8_t row = 0; row < STATE_ROWS; row++ ) {
...
int test_addRoundKey() {
  ...
  setCurrentState(&afterAddRoundKeyState[0][0]);
  invAddRoundKey(roundKey);
  for ( uint8_t col = 0; col < STATE_COLS; col++ ) {
    for ( uint8_t row = 0; row < STATE_ROWS; row++ ) {
...
int test_invCipher() {
  printf("-- Testing test_invCipher ... ");
  uint8_t output[STATE_COLS][STATE_ROWS];
  invCipher(&outputState[0][0], &output[0][0], expandedCipherKey, NR_128);
  for ( uint8_t col = 0; col < STATE_COLS; col++ ) {
    for ( uint8_t row = 0; row < STATE_ROWS; row++ ) {
      if (output[col][row] != inputState[col][row]){
        printf("failure\n");
        return 0;
      }
    }
